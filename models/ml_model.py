    ...
    def _analyze_features(self, values: Mapping[str, float]) -> list[dict]:
        ...
        for key, value in values.items():
            ...
            if key == "sofa":
                ...
                status_text = "Thấp so với ngưỡng hiển thị" if status == "danger" else "Trong/ trên ngưỡng hiển thị"
                normalized = min(100, max(0, value / normal_max * 100))
            else:
                status = "danger" if value > normal_max else "normal"
                status_text = "Cao so với ngưỡng hiển thị" if status == "danger" else "Trong/ dưới ngưỡng hiển thị"
